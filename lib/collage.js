...
export function collageFinal({ frases = [], lunas = 9 }) {
  ...
  ctx.fillText("ME PARÍ A MÍ MISMA", w / 2, 120);
  // fila de 9 lunas crecientes
  const y = 250, r = 42, gap = (w - 160) / (lunas - 1);
  for (let i = 0; i < lunas; i++) luna(ctx, 80 + i * gap, y, r, (i + 1) / lunas);
  ctx.fillStyle = TINTA; ctx.font = "italic 600 76px 'Cormorant Garamond', Georgia, serif";
  ctx.fillText("9 lunas", w / 2, 430);
  // frases del diario

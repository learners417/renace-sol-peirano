...
export const logout = () => { if (typeof window === "undefined") return; try { TODAS_LAS_CLAVES.forEach((k) => localStorage.removeItem(k)); ["companera","dialogo","juego"].forEach((m) => localStorage.removeItem("rn_serena_" + m)); } catch {} };
...
export const promedioInicial = () => {
  ...
  const b = ob?.bases;
  if (b) { const vs = Object.values(b); return Math.round((vs.reduce((a, x) => a + x, 0) / vs.length) * 10) / 10; }
  return ob?.termometro ?? null;
};
export const getBases = () => getOnboarding()?.bases || null;
export const guardarBases = (bases) => { const ob = getOnboarding() || {}; set("rn_onboarding", { ...ob, bases, fecha: ob.fecha || Date.now() }); };
export const baseDeArea = (n) => { const b = getBases(); return b?.[n] ?? null; };
...
export const areaScore = (n) => {
  const base = baseDeArea(n);
  if (base != null) return Math.min(100, Math.round(base * 10 + moduloProgreso(n) * 15 + Math.min(30, pesoDeArea(n) * 8)));
  return Math.min(100, Math.round(moduloProgreso(n) * 30 + Math.min(70, pesoDeArea(n) * 12)));
};
export const nivelArea = (n) => Math.round(areaScore(n)) / 10;
export const nivelRenacimiento = () => {
  let t = 0; for (let n = 1; n <= 9; n++) t += areaScore(n);
  return Math.round(t / 9) / 10;
};
...
export const esFinDeSemana = () => new Date().getDay() === 0; // domingo: el día libre de la semana

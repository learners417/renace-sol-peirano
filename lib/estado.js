...
export const agregarDiario = (texto, animo, luna) => {
  const d = getDiario();
  const n = luna ?? lunaActual();
  d.unshift({ texto: (texto || "").trim(), animo: animo ?? null, luna: n, fecha: Date.now() });
  set("rn_diario", d);
  return d;
};
...
export const agregarHito = ({ texto, area, animo, foto }) => {
  const h = getHitos();
  h.unshift({ texto: (texto || "").trim(), area: area || 1, animo: animo ?? null, foto: foto || null, fecha: Date.now() });
  set("rn_hitos", h);
  return h;
};
...
export const areaScore = (n) => Math.min(100, Math.round(moduloProgreso(n) * 40 + Math.min(60, hitosDeArea(n).length * 20)));
